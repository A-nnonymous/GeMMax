...
def load_and_prepare_data(csv_path, gemm_type='trivial'):
    ...
    try:
        ...
        if gemm_type == 'trivial':
            required_cols = ['m', 'k', 'n', 'time_us', 'throughput_TFLOPS', 'bandwidth_GBs']
        else:  # grouped
            required_cols = ['num_groups', 'valid_m', 'k', 'n', 'time_us', 'throughput_TFLOPS', 'bandwidth_GBs']
...
def plot_3d_surface(ax, K, N, Z, title, metric_label, cmap='coolwarm'):
    ...
    if K is None or N is None or Z is None:
        ax.text(0.5, 0.5, 0.5, 'Insufficient data', 
                ha='center', va='center', transform=ax.transAxes)
        return None
    
    surf = ax.plot_surface(K, N, Z, cmap=cmap,
                          linewidth=0, antialiased=True, alpha=0.8)
    
    z_range = Z.max() - Z.min()
    z_offset = Z.min() - 0.1 * z_range if z_range > 0 else Z.min() - 1
    ax.contourf(K, N, Z, zdir='z', offset=z_offset, 
                cmap=cmap, alpha=0.5)
...
def plot_trivial_gemm(csv_path, output_dir='./results/gemm/trivial'):
    """绘制trivial GEMM性能图表"""
...
def plot_grouped_gemm(csv_path, output_dir='./results/gemm/grouped', layout='auto'):
    ...
    for valid_m in sorted(df['valid_m'].unique()):
        ...
        fig.suptitle(f'Performance Metrics for valid_m={valid_m} ({n_groups} groups)', fontsize=16)
        ...
        for i, num_groups in enumerate(num_groups_list):
            ...
            for j, (metric, title) in enumerate(metrics_info.items()):
                ...
                if len(group_data) < 4:
                    ...
                    surf = plot_3d_surface(ax, K, N, Z, 
                                         f'G={num_groups}: {title}', 
                                         title)
...
def plot_comparison(trivial_csv, grouped_csv, output_dir='./results/gemm/comparison'):
    ...
    metrics_info = {
        ...
        'throughput_TFLOPS': ('Throughput Difference (%)', 'RdBu', 'higher'),  # 吞吐量越高越好
    ...
    for m in common_m_values:
        ...
        fig = plt.figure(figsize=(fig_width, fig_height))
        fig.suptitle(f'Performance Comparison: Grouped vs Trivial (m={m})\n' + 
                    'Positive values (red) indicate grouped is better', fontsize=16)
        ...
        for num_groups in num_groups_list:
            ...
            # 合并数据以确保相同的(k,n)点
            merged = pd.merge(
                trivial_data[['k', 'n', 'time_us', 'throughput_TFLOPS', 'bandwidth_GBs']],
            ...
            if len(merged) < 4:
                for j in range(3):
                    ax = fig.add_subplot(rows, cols, plot_idx)
                    ax.text(0.5, 0.5, f'Insufficient matching data\nfor num_groups={num_groups}', 
                           ha='center', va='center', transform=ax.transAxes)
                    ax.set_title(f'G={num_groups}')
                    plot_idx += 1
                continue
            ...
            for metric, (title, cmap, better) in metrics_info.items():
                ...
                if metric == 'time_us':
                    ...
                    # 正值表示grouped更快
                    merged['diff'] = (merged[f'{metric}_trivial'] - merged[f'{metric}_grouped']) / merged[f'{metric}_trivial'] * 100
                else:
                    # 吞吐量和带宽: (grouped - trivial) / trivial * 100
                    # 正值表示grouped更好
                    merged['diff'] = (merged[f'{metric}_grouped'] - merged[f'{metric}_trivial']) / merged[f'{metric}_trivial'] * 100
                
                # 插值差异数据
                diff_data = merged[['k', 'n', 'diff']]
                K, N, Z = interpolate_metric(diff_data.rename(columns={'diff': 'metric'}), 'metric')
