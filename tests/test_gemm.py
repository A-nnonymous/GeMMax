...
#m_list = [64, 128, 1024, 2048, 4096, 32768]
m_list = [128,  512, 2048, 8192, 32768]
...
def test_gemm_to_csv():
    ...
    rows = []
    headers = ['m', 'k', 'n', 'time_us', 'throughput_TFLOPS', 'bandwidth_GBs']
    ...
    for m, n, k in itertools.product(m_list, n_list, k_list):
        ...
        t = bench_kineto(test_func, 'fp8_gemm', suppress_kineto_output=True)
        throughput = 2 * m * n * k / t / 1e12
        bandwidth = (m * k + k * n + m * n * 2) / 1e9 / t
        rows.append([m, k, n, t * 1e6, throughput, bandwidth])

    write_to_csv('gemm_results.csv', headers, rows)
    print()
...
def test_m_grouped_gemm_contiguous() -> None:
    ...
    for num_groups, m, n, k in itertools.product(num_groups_list, m_list, n_list, k_list):
...
def test_m_grouped_gemm_contiguous_to_csv():
    ...
    rows = []
    headers = ['num_groups', 'expected_m_per_group', 'n', 'k', 'valid_m', 'time_us', 'throughput_TFLOPS', 'bandwidth_GBs']

    for num_groups, m, n, k in itertools.product(num_groups_list, m_list, n_list, k_list):
        expected_m_per_group = m // num_groups
        if expected_m_per_group < 128: continue
        m, x_fp8, y_fp8, m_indices, out, ref_out = construct_contiguous_grouped(num_groups, expected_m_per_group, k, n)
        deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_contiguous(x_fp8, y_fp8, out, m_indices)
        out = torch.where((m_indices == -1).unsqueeze(1), torch.zeros_like(out), out)
        #diff = calc_diff(out, ref_out)
        ...
        t = bench_kineto(test_func, 'fp8_gemm', suppress_kineto_output=True)
        valid_m = (m_indices != -1).sum().item()
        print(f"Running case: num_groups={num_groups}, expected_m_per_group={expected_m_per_group}, n={n}, k={k}, valid_m={valid_m}")
        throughput = 2 * valid_m * n * k / t / 1e12
        bandwidth = (valid_m * k + num_groups * k * n + valid_m * n * 2) / 1e9 / t
        rows.append([num_groups, expected_m_per_group, n, k, valid_m, t * 1e6, throughput, bandwidth])

    write_to_csv('grouped_contiguous_gemm_results.csv', headers, rows)
    print()

def test_m_grouped_gemm_masked() -> None:
    print('Testing grouped masked GEMM:')

    for num_groups_list, m, n, k in itertools.product(num_groups_list, m_list, n_list, k_list):
...
def test_m_grouped_gemm_masked_to_csv():
    ...
    for num_groups, m, n, k in itertools.product(num_groups_list, m_list, n_list, k_list):
        ...
        # Test performance with fixed shapes
        def test_func():
            x_fp8, y_fp8, masked_m, out, _ = construct_masked_grouped(num_groups, 4096, expected_m_per_group, k, n)
            deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_masked(x_fp8, y_fp8, out, masked_m, expected_m_per_group)
    ...
    write_to_csv('grouped_masked_gemm_results.csv', headers, rows)
    print()
...
if __name__ == '__main__':
    ...
    test_gemm_to_csv()
    test_m_grouped_gemm_contiguous_to_csv()
    #test_m_grouped_gemm_masked_to_csv()
    '''
    test_gemm()
