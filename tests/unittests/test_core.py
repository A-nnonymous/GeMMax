...
def test_gemm() -> None:
    ...
    for m in (64, 128, 4096):
        for k, n in [
            ...
            print(
                f" > Perf (m={m:5}, n={n:5}, k={k:5}): {t * 1e6:4.0f} us | "
...
def test_m_grouped_gemm_contiguous() -> None:
    ...
    for num_groups, expected_m_per_group, k, n in (
        ...
        print(
            f" > Perf ({num_groups=:2}, {expected_m_per_group=:4}, n={n:4}, k={k:4}): {t * 1e6:4.0f} us | "
...
def test_m_grouped_gemm_masked() -> None:
    ...
    for num_groups, expected_m_per_group in ((1, 1024), (2, 512), (4, 256)):
        for k, n in (
            ...
            t = bench_kineto(test_func, "fp8_gemm", suppress_kineto_output=True)
            print(
                f" > Perf ({num_groups=}, expected_m_per_group={expected_m_per_group:4}, n={n:4}, k={k:4}): {t * 1e6:4.0f} us | "
                f"throughput: {2 * valid_m * n * k / t / 1e12:4.0f} TFLOPS, "
                f"{(valid_m * k + num_groups * k * n + valid_m * n * 2) / 1e9 / t:4.0f} GB/s"
            )
    print()
...
def test_wgrad_gemm():
    ...
    for k in (4096, 8192):
        for m, n in (
            ...
            print(
                f" > Performance (m={m:5}, n={n:5}, k={k:5}): {t * 1e6:4.0f} us | "
                f"throughput: {2 * m * n * k / t / 1e12:4.0f} TFLOPS, "
                f"{(m * k + k * n + m * n * 2) / 1e9 / t:4.0f} GB/s"
            )
    print()
...
def test_k_grouped_wgrad_gemm():
    ...
    for num_groups, base_k in ((4, 4096), (4, 8192), (8, 4096)):
        for m, n in ((7168, 4096), (2048, 7168)):
            ...
            print(
                f" > Performance ({num_groups=}, m={m:5}, n={n:5}, avg_k={total_k//num_groups:5}): {t * 1e6:4.0f} us | "
